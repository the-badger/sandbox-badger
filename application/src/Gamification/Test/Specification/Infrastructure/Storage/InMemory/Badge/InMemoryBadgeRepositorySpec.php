...
class InMemoryBadgeRepositorySpec extends ObjectBehavior
{
    ...
    public function it_gets_a_badge()
    {
        ...
        $this->get($badge->id())->shouldBeABadgeOptionLike(BadgeOption::some($badge));
    }
    ...
    public function it_gets_a_badge_by_its_title()
    {
        $badge = $this->badge();
        $this->save($badge)->shouldReturn(null);
        $this->getBadgeByTitle($badge->title())->shouldBeABadgeOptionLike(BadgeOption::some($badge));
    }
    ...
    {
        return new Badge(new BadgeId(Uuid::uuid5(Uuid::NAMESPACE_DNS, 'badger_id')), BadgeTitle::fromString('A Badge title'), BadgeDescription::fromString('Your awesome badge description!'), BadgePoints::fromInt(50));
    }
    ...
    function getMatchers(): array
    {
        return [
            'beABadgeOptionLike' => function (BadgeOption $subject, BadgeOption $value) {
                ...
                return $subject->badge() === $value->badge();
            }
        ];
    }
}
